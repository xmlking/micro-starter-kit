...
#RUN CGO_ENABLED=0 GOOS=linux GOARCH=amd64 go build -a -o cmd/great1/great1.go
#RUN GOOS=darwin GOARCH=amd64 go build -v -o /bin/great1 cmd/great1/great1.go
ARG VERSION=0.0.1
ARG BUILD_PKG="./srv/account"
RUN echo $(govvv -flags -version ${VERSION} -pkg $(go list ./shared/config) )
RUN go build -a \
...
LABEL org.label-schema.build-date=$BUILD_DATE \
    ...
    org.label-schema.docker.cmd="docker run -it -p 80:8080  ${IMANGE_NAME}"
